...
const WorkButton = ({item, isSelected, selectedWorkId, onClick}:WorkButtonProps) => {
  ...
  > 
    ...
    <button className={`detail-button absolute bottom-5 right-5 h-7 rounded-full overflow-hidden bg-white bg-opacity-30 hover:bg-opacity-50 xl:flex hidden items-center pl-[9px] backdrop-blur transition-all duration-300 box-shadow-2 ${selectedWorkId !== -1 && selectedWorkId !== item.id && "opacity-0"}`}>
